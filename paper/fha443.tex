...
We build that extractor and measure how far it gets. FHA-443 holds 757 canonicalized Nature of Suit 443 opinion clusters, 751 with full text and 417 rule-positive and substantive. A deterministic layer labels five doctrinal constructs and the proof framework, attaching a character offset and a snippet to every flag, and a frozen three-pass Claude Opus 4.8 baseline given the same codebook provides the comparison. Against 93 blind hand-coded opinions the model route is better on every aggregate measure, and the whole of that gain sits in one construct. Rule-based disparate-impact detection runs at precision 0.41 for exactly the reason above, and requiring an adjudicating quote lifts it to 1.00 without costing recall. Refusal and steering defeat both routes, which we read as heterogeneity in how courts describe the conduct rather than a limit of either method.
...
Because NOS coding is a docketing convention rather than a merits label, canonical cases can mix incidental references with FHA decisions, and citations may appear in procedural history. The rule therefore requires 200 characters plus a core citation or corroborating claim or remedy evidence. Equation~\ref{eq:inclusion} is a high-recall selection frame within the retrieval; writing $L_i$ for text length, $N_i$ for the NOS code, $C_i$ for a core FHA citation, $A_i$ for an Act-name match, and $Q_i$ for the count of corroborating claim or remedy cues,
\begin{equation}\label{eq:inclusion}
\resizebox{0.89\columnwidth}{!}{$
I_i = \mathbf{1}\{L_i \ge 200\}\cdot\mathbf{1}\{N_i = 443\}\cdot\mathbf{1}\{C_i \lor [A_i \land (Q_i \ge 2)]\}
$}
\end{equation}
Hand-coded validation evaluates downstream indicators, so 417 is a rule-positive denominator.

Full text is recovered for 751 of 757 canonical clusters (99\%), and the remaining six lack retrievable text. The original 937-row snapshot and 180 excluded candidates remain in the repository, so the retained sample is this CourtListener district-court retrieval. The representativeness comparison concerns retrieval composition only. Section~\ref{sec:validation} and Table~\ref{tab:gold} report the hand-coded set and the construct-specific accuracy it supports.

\section{Methods}

\subsection{Structured legal-variable extraction}\label{sec:extraction}

The task is multi-label span-grounded classification over a full opinion. The input is the text of one district-court opinion cluster. The output is a record with six binary construct labels, disparate treatment, disparate impact, refusal or steering, reasonable accommodation, exclusionary zoning, and an outcome cue, one categorical proof-framework label, and for every positive flag a character span and the quoted passage that licenses it. Labels are nonexclusive, because an opinion can adjudicate a duty and a course of conduct at once. The span requirement is what separates an opinion that recites a standard from one that applies it, and it is what makes a false positive findable by a reader rather than only by a second annotator.

From each opinion, deterministic regex and lexicon rules create a structured legal record (Appendix~\ref{app:pipeline}). Each label names a doctrine, and the doctrine fixes what counts as a positive. Disparate treatment is intentional differential handling of a housing transaction, and at the trial level it is usually litigated through the burden-shifting structure of \citet{mcdonnell1973}. Disparate impact reaches a facially neutral practice through its effects, on the logic of \citet{griggs1971}, recognized under the Fair Housing Act in \citet{inclusive2015} and implemented by HUD in 2013 \citep{hud2013} and 2023 \citep{hud2023}. Reasonable accommodation is the disability-related duty in 42 U.S.C.\ \S~3604(f)(3)(B) \citep{fha1968}, so it can co-occur with either theory. Refusal or steering and exclusionary zoning describe conduct, by a private actor and by a municipality. The labels sit at three levels, proof, duty, and conduct, and the schema keeps them separate rather than forcing one winner per opinion. The evidence span exists to record whether an opinion applies a doctrine or only recites it, a live instance of the gap between the law as written and the law as applied \citep{pound1910}.
...
\midrule
Disp.\ treatment & 0.910 & 0.720 & 0.804 & 0.60 \\
Disp.\ impact & 0.407 & 0.917 & 0.564 & 0.47 \\
Refusal to rent/sell & 0.731 & 0.613 & 0.667 & 0.52 \\
Reasonable accomm. & 0.829 & 0.944 & 0.883 & 0.80 \\
Exclusionary zoning & 0.800 & 0.800 & 0.800 & 0.76 \\
\midrule
Micro-average & 0.760 & 0.780 & 0.767 & 0.66 \\
\bottomrule
\end{tabular}
...
Accuracy varies across constructs (Table~\ref{tab:gold}). Disparate-impact detection recovers 0.92 of hand-coded positives at precision 0.41, so its $F_1$ of 0.56 suits screening while its false-positive rate limits direct prevalence claims. Framework accuracy is 0.77 ($\kappa=0.47$), holding-cue detection is 0.71/0.59/0.64, and binary agreement on 37 decisive cases is 0.81 ($\kappa=0.61$). The 30 double-coded cases show claim $\kappa=0.90$, framework $\kappa=0.86$, and four-way-winner $\kappa=0.73$.
...
\end{equation}
micro precision, recall, and $F_1$ remain 0.76, 0.78, and 0.77, and the treatment--impact ordering survives.

\subsection{LLM baseline and error correction}\label{sec:llm}

To test regex and lexicon behavior against a stronger extractor, we froze a Claude Opus 4.8 comparison. The model received the same human codebook and evidence-quote requirement used in annotation, and returned the five claim labels and the framework field in three independent passes; we report the majority vote. Across the 1,215 claim decisions on the overlap and draw, 98.8\% were unanimous. That figure records sampling stability under a fixed prompt, not agreement between independent coders. The run covers the 93-case human overlap and a uniform 150-case draw, seeded 20260720, from the 658 clusters outside the human set, with long opinions truncated head-and-tail. Of 729 requested passes, 728 returned, and the single two-pass case agreed on all six fields. One model at one setting measures what Opus 4.8 recovers, not a property of LLM extraction.
...
\midrule
Disp.\ treatment & .907/.722/.804 & .867/.963/.912 & $+$.108 \\
Disp.\ impact & .407/.917/.564 & 1.00/.917/.957 & $+$.393 \\
Refusal/steering & .731/.613/.667 & .680/.548/.607 & $-$.060 \\
Reas.\ accomm. & .829/.944/.883 & 1.00/.944/.971 & $+$.088 \\
Excl.\ zoning & .800/.800/.800 & .833/1.00/.909 & $+$.109 \\
\midrule
Micro average & .757/.777/.767 & .872/.872/.872 & $+$.105 \\
Framework acc. & \multicolumn{1}{c}{.774} & \multicolumn{1}{c}{.914} & $+$.140 \\
\bottomrule
\end{tabular}
...
The trial-court result is a treatment lead with a method-sensitive accommodation estimate. The precision-recall correction gives 47.2\% treatment, 31.2\% accommodation, 12.5\% impact, 28.3\% refusal, and 9.4\% zoning. The LLM draw and the correction are two independent error models, a stronger extractor on a fresh random draw and a measurement-error correction of the rule system (Figure~\ref{fig:corrections}). They agree on treatment, impact, and zoning, which is evidence the treatment lead is not an artifact of either route.
...
Because circuits differ in claim mix, a raw enforcement ranking conflates doctrine with docket composition. Raw and case-mix-adjusted circuit ranks correlate only 0.14, and the ordering shifts under reweighting, so case-mix adjustment is a requirement rather than a refinement for any cross-circuit index. Appendix~\ref{app:feii} reports the index components by circuit, which show that a circuit high on opinion volume need not rank high on remedy-cue intensity.
...
Separating the legal signal, cross-group access, and the household preference rule keeps each layer available to a design that can identify a housing effect, which is what places a measured institutional signal upstream of a segregation mechanism. The released schema records evidence for proof, duty, conduct, framework, remedy, and outcome-cue indicators. The release also includes an unsupervised embedding of the reasoning text (TF--IDF, latent semantic analysis \citep{deerwester1990}, and $k$-means \citep{lloyd1982}) that recovers coarse textual structure but does not enter the sorting analysis.
...
Everything needed to rebuild the numbers above is in \repowhere, and one command rebuilds them: \texttt{make reproduce} runs the pipeline, the gold-set scoring, the frozen-LLM comparison, the prevalence correction, the circuit split, and the model sweep offline, with no API call. The archive holds the 937 candidate records and 180 exclusions, the canonical cluster identifiers, the extraction rules with their lexicons, the annotation codebook, the 93 human labels with the 30-case second pass, the frozen prompt, the three per-pass label sets from the Opus 4.8 run, and the draw seed 20260720. We release identifiers and derived labels rather than opinion text, and the retrieval script reconstructs the text from CourtListener. Model outputs are from a single frozen checkpoint and a fixed prompt; the prompt, the truncation rule, and the pass count are in the release so the run can be repeated or replaced with a different model. The pipeline runs nine ordered stages and records inputs and exclusions at each (Appendix~\ref{app:pipeline}). A closing check rebuilds the tables printed here from those outputs and compares every number the paper states against the value the code produces, failing the build when the two disagree, so a change to the pipeline cannot leave a stale figure standing in the text. Results hold at their defined levels: 417 clusters for textual composition, 76 random-draw cases for the LLM comparison, 93 overlap cases for construct diagnostics, and eight one-year cells for feasibility.

\section{Conclusion}

FHA-443 makes the federal trial-level Fair Housing Act docket measurable, and measuring it changes what the docket looks like. Rule-based extraction makes the three leading theories appear nearly tied; an extractor required to quote the adjudicating passage puts disparate treatment clearly first and cuts disparate-impact false positives to zero on the human overlap. Refusal and steering remain unsolved under both routes, which is the open problem this corpus hands to the next paper. Circuits split on the doctrine they apply, so a cross-circuit index built on this text has to be case-mix adjusted. We release the corpus, the extractor, the codebook, and the labels, and we defer the housing question to a design that can measure legal exposure, household preferences, and access together.
...
The corpus consists of published federal court opinions, which are public records and carry no expectation of privacy. Party names appear in the source text as the courts published them. We redistribute opinion identifiers and derived labels rather than re-hosting opinion text, and the retrieval script reconstructs the text from CourtListener under that project's terms.
...
One deterministic forward pass runs nine ordered stages, each consuming the prior stage's output without feedback: harvest and identify from CourtListener bulk data with the excluded-candidate audit preserved; identify substantive FHA by the deterministic rule of Equation~\ref{eq:inclusion}; extract variables with evidence spans; aggregate the enforcement index; merge housing feasibility inputs; check estimability, recording an insufficient-panel note and fitting no real-data panel model; run validation diagnostics; run the frozen LLM baseline; and sweep the gatekeeping scenarios on a seeded grid. Stochasticity is seeded throughout. After the ninth stage, a table builder reads the written outputs, emits the tables reported above, and asserts each headline number against the paper, exiting non-zero on any disagreement. A released breadth score,
\begin{equation*}
\resizebox{0.89\columnwidth}{!}{$
...
The model initializes a $20\times20$ Moore-neighborhood grid with two equally sized groups and 10\% vacancies. An agent accepts a vacant cell only when its same-type neighbor share meets a preference threshold $\tau$. Unsatisfied agents and a 4\% turnover draw seek new cells, and a separate gatekeeping rule can block a cross-group move that otherwise meets $\tau$ (Figure~\ref{fig:gates}):
\begin{equation}\label{eq:access}
\resizebox{0.89\columnwidth}{!}{$
\Pr(\text{denial}\mid\text{cross-group move}) = g(1-e),\quad e \in \{.15,.325,.50,.675,.85\},\ g=.80
$}
\end{equation}
We sweep $\tau$ and the access level $e$ independently, using the observed circuit-mean index span as legal context for the five access scenarios, and run 250 update cycles and 40 replications per cell with a fixed seed.

\begin{figure}[h]
\centering
\includegraphics[width=0.86\columnwidth]{media/image18.png}
\caption{Two sequential gates: a preference threshold $\tau$, then a cross-group access barrier.}
\label{fig:gates}
\end{figure}

At $\tau=0.20$, the mean same-type neighbor share falls from 0.680 at access 0.15 to 0.605 at 0.85. At $\tau=0.30$ it falls from 0.797 to 0.752, a smaller drop, because the higher threshold admits fewer cross-group moves. At $\tau=0.45$ no cross-group move clears the threshold and the share stays near 0.90, so widening access changes nothing (Figure~\ref{fig:surface}). The asymmetry is a property of the model, not a finding about American housing: access moves sorting only where the preference rule permits cross-group moves, so the preference rule bounds the effect legal access can have.
...
\caption{FHA measurement and inference pipeline.}
\label{fig:pipeline}
\end{figure}

\section{Annotation codebook}\label{app:codebook}

Human coders and the LLM baseline received the same instructions, reproduced in condensed form here; the release carries them verbatim. The core rule is scoping: code what the opinion itself adjudicates or substantively discusses as an asserted FHA theory, and never code a claim because a phrase appears in a string-cited case name, a quoted statute, or procedural history. Every positive label requires a short verbatim quote from the opinion, and every judgment rests on the opinion text alone.
